    ...
    if (boolean_inCard == false) { 
        ...
        for (let eventType in action_tables) {
            ...
            if (t_event == t_triggeredBy) {
                ...
                show_alert(temp_event, temp_event_txt, t_location)
            }
        }
        boolean_inCard = false
    }
...
async function async_playQueue() {
    ...
    if (arr_currentQueue.length > 0) {
        for (let i = 0; i < arr_currentQueue.length; i++) {
            ...
            }else if (t_desc.includes(`particularly dangerous situation`) && t_event == `Tornado Warning`) {
                debug(`NOTIFY | ${t_event} >> ${t_location} playing...`)
        ...
        fetch(`${home_ip_and_port}/api/manual`, {headers: global_header}).then(response => response.text()).then(data => {
            ...
            if ( arr_json.eventType != undefined) {
                ...
                fetch(`${home_ip_and_port}/api/archive`, {headers: global_header}).then(response => response.text()).then(data => {
                    ...
                    update_all_ui(arr_ActiveWarnings)
...
setInterval(() => {
    ...
    let tornado_emergency = false 
    let tornado_pds = false
    let tornado_warning = false 
    let svr_warning = false
    let flash_flood = false
    let special_marine = false
    if (warnings.length != 0) {
        for (let i = 0; i < warnings.length; i++) {
            if (warnings[i].event.includes(`Tornado Emergency`)) { tornado_emergency = true; }
            if (warnings[i].event.includes(`PDS`)) { tornado_pds = true; }
            if (warnings[i].event.includes(`Tornado Warning`)) { tornado_warning = true; }
            if (warnings[i].event.includes(`Severe Thunderstorm Warning`)) { svr_warning = true; }
            if (warnings[i].event.includes(`Flash Flood Warning`)) { flash_flood = true; }
            if (warnings[i].event.includes(`Special Marine`)) { special_marine = true; }
        }
        if (tornado_emergency == true) { 
            get_light = document.getElementsByClassName(`defaultBoxLight`)
            get_dark = document.getElementsByClassName(`defaultBox`)
            for (let x = 0; x<get_light.length; x++){
                get_light[x].style.backgroundColor = `rgb(209,38,215)`
            }
            for (let x = 0; x<get_dark.length; x++){
                get_dark[x].style.backgroundColor = `rgb(159,37,163)`
            }
        }else if (tornado_pds == true) { 
            get_light = document.getElementsByClassName(`defaultBoxLight`)
            get_dark = document.getElementsByClassName(`defaultBox`)
            for (let x = 0; x<get_light.length; x++){
                get_light[x].style.backgroundColor = `rgb(249,56,54)`
            }
            for (let x = 0; x<get_dark.length; x++){
                get_dark[x].style.backgroundColor = `rgb(203,25,25)`
            }
         }else if (tornado_warning == true) { 
            get_light = document.getElementsByClassName(`defaultBoxLight`)
            get_dark = document.getElementsByClassName(`defaultBox`)
            for (let x = 0; x<get_light.length; x++){
                get_light[x].style.backgroundColor = `rgb(249,56,54)`
         ...
         }else if (svr_warning == true) { 
            get_light = document.getElementsByClassName(`defaultBoxLight`)
            get_dark = document.getElementsByClassName(`defaultBox`)
            for (let x = 0; x<get_light.length; x++){
                get_light[x].style.backgroundColor = `rgb(224,162,42)`
            }
            for (let x = 0; x<get_dark.length; x++){
                get_dark[x].style.backgroundColor = `rgb(255,132,1)`
            }
         }else if (flash_flood == true) { 
           get_light = document.getElementsByClassName(`defaultBoxLight`)
           get_dark = document.getElementsByClassName(`defaultBox`)
           for (let x = 0; x<get_light.length; x++){
               get_light[x].style.backgroundColor = `rgb(102,209,60)`
           }
           for (let x = 0; x<get_dark.length; x++){
               get_dark[x].style.backgroundColor = `rgb(73,155,40)`
           }
        }else if (special_marine == true) { 
            get_light = document.getElementsByClassName(`defaultBoxLight`)
            get_dark = document.getElementsByClassName(`defaultBox`)
            for (let x = 0; x<get_light.length; x++){
                get_light[x].style.backgroundColor = `rgb(28,61,181)`
            }
            for (let x = 0; x<get_dark.length; x++){
                get_dark[x].style.backgroundColor = `rgb(42,81,224)`
            }
        }else{
           get_light = document.getElementsByClassName(`defaultBoxLight`)
           get_dark = document.getElementsByClassName(`defaultBox`)
