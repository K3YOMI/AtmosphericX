...
geolocation.plot = async function(latest) {
    let location = latest['locations'].split(',')[0]
    let state = latest['locations'].split(',')[1].split(';')[0]
    fetch('https://nominatim.openstreetmap.org/search?format=json&q=' + location + ', ' + state).then(response => response.json()).then(data => {
        ...
        let circle = L.circle([lat, lon], {color: 'black',fillColor: 'red',fillOpacity: 0.1,radius: 50000}).addTo(geolocation.cache.map).bindPopup(`<b>${latest['eventName']} (${latest['messageType']})</b><br>${latest['locations']}`).openPopup();
