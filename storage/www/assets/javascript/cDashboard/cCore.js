...
class Dashboard { 
    constructor(library, inDashboard=false) {
    ...
    triggerWidgetListener = function() {
        ...
        this.injectNotification({
            ...
            parent: `_body.base`, 
            buttons: [
                { name: `Spawn Alert`, className: `button-ok`, function: () => { fetch(`/api/manual`, {method: `POST`, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ event: document.getElementById(`widget_alert_type`).value, properties: { senderName:"Manual Admin Override", event: document.getElementById(`widget_alert_type`).value, description: `This is a test alert. A ${document.getElementById(`widget_alert_type`).value} has been issued for parts of ${document.getElementById(`widget_notification_alert_location`).value}`, messageType: document.getElementById(`widget_alert_status`).value, expires: "N/A", indicated: document.getElementById(`widget_alert_indicator`).value, areaDesc: document.getElementById(`widget_notification_alert_location`).value, parameters: {} } }), })}},
    ...
    spawnAlertCards = function(domDirectory=`child_atmosx_alerts.global_alerts`, recentOnly=false, searchBar=`_alerts.alert_search`, searchTerm=``) {
        ...
        for (let i = 0; i < maxShownAlerts; i++) {
            ...
            let eventIssued = library.getTimeInformation(alert.details.issued == undefined ? `No date found` : alert.details.issued)
            ...
            this.injectCardData({
                ...
                onclick: () => {
                    ...
                    for (let i = 0; i < eventHistory.length; i++) {
                        ...
                        eventHistoryString += `\n\n${genericDashline} ${descriptionSegement.action} (${descriptionSegement.issued}) ${genericDashline}\n\n${descriptionSegement.description}`;  
                    }
                    if (eventHistoryString == ``) { eventHistoryString = currentDescription } 
                    let eventSubtitle = `Event: ${eventName} (${eventStatus})<br>Locations: ${alert.details.locations}<br>Issued: ${eventIssued.formal}<br>Expires in: ${time.expires}<br>Wind Gust: ${maxWindGust} <br>Hail: ${maxHailSize}<br>Damage Threat: ${damageThreat}<br>Tornado: ${tornadoIndicator}<br>Tag: ${eventTags}<br>Sender: ${fullSendName}<br>Distance: ${distanceAway}<br>Tracking ID: ${eventTrackingID}`
                    this.injectNotification({
                        title: `${eventName} (${eventStatus})`,  
