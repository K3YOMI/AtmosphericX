...
class library {
    ...
    async getConfigurations() {
        let ipAddress = this.getAddress();
        return new Promise(async (resolve, reject) => {
            await fetch(`${ipAddress}/api/configurations`).then(response => response.text()).then(data => {
...
class stream { 
    ...
    async updateGeneralListings(warningList) {
        ...
        const warningTypes = [
            { type: 'totalTornadoEmergencies', color: { light: 'rgb(209,38,215)', dark: 'rgb(159,37,163)' }, count: totalTornadoEmergencies },
            { type: 'totalPDS', color: { light: 'rgb(249,56,54)', dark: 'rgb(203,25,25)' }, count: totalPDS },
            { type: 'totalTornadoWarnings', color: { light: 'rgb(249,56,54)', dark: 'rgb(203,25,25)' }, count: totalTornadoWarnings },
            { type: 'totalFlashFloodEmergencies', color: { light: 'rgb(249,56,54)', dark: 'rgb(203,25,25)' }, count: totalFlashFloodEmergencies },
            { type: 'totalSnowSquallWarnings', color: { light: 'rgb(249,56,54)', dark: 'rgb(203,25,25)' }, count: totalSnowSquallWarnings },
            { type: 'totalSevereThunderstormWarnings', color: { light: 'rgb(224,162,42)', dark: 'rgb(255,132,1)' }, count: totalSevereThunderstormWarnings },
            { type: 'totalFlashFloodWarnings', color: { light: 'rgb(102,209,60)', dark: 'rgb(73,155,40)' }, count: totalFlashFloodWarnings },
            { type: 'totalSpecialMarineWarnings', color: { light: 'rgb(28,61,181)', dark: 'rgb(42,81,224)' }, count: totalSpecialMarineWarnings },
            { type: 'totalNoWarnings', color:{ light: 'rgb(28,61,181)', dark: 'rgb(42,81,224)' }, count: 0 }
        ];
        let highest;
        highest = warningTypes.find(type => {
            return type.count > 0;
        }) || warningTypes[warningTypes.length - 1];
        for (let x = 0; x<lightAreas.length; x++){
            lightAreas[x].style.backgroundColor = highest.color.light
        }
        for (let x = 0; x<darkAreas.length; x++){
            darkAreas[x].style.backgroundColor = highest.color.dark
        }
    ...
    async runQuery(queryData) {
        ...
        if (autoBeep) { 
            lib.playsound(config['DEFINED_SOUNDS']['BEEP']);
            if (!onlyBeep) {await lib.delay(1300); lib.playsound(audioToUse)}
        }else{
            lib.playsound(audioToUse)
        }
        if (eas || siren) {
            await lib.delay(3500);
            lib.playsound(eas ? config['DEFINED_SOUNDS']['EAS'] : config['DEFINED_SOUNDS']['TORE']);
        }
        if (!streamModeEnabled) {
