...
class AlertBuilder { 
    ...
    rawTextAlert = async function(metadata) {
        ...
        for (let i = 0; i < message.length; i++) {
            ...
            if (vtec && ugc) {
                ...
                let senderOffice = loader.modules.raw.getOfficeName(msg) || vtec.trackingId.split(`-`)[0];
                ...
                if (dateLineMatches.length) {
                    ...
                    if (nwsStart !== -1) {
                        ...
                        msg = latStart !== -1 ? msg.substring(nwsStart + dateLineMatch[0].length, latStart).trim() : msg.substring(nwsStart + dateLineMatch[0].length).trim();
    ...
    xmlAlert = async function(metadata) {
        ...
        let xmlData = new loader.packages.xml2js.Parser();
